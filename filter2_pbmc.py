...
#remove single exon transcripts < 2000 and multi exon transcripts < 200
iuxgtf_list = []
...
for entry in iuxgtf_list:
    if entry[2] == "transcript":
        ...
        if (len(exons) == 1) and (exons[0] > 2000):
            singleexon.append(entry)
        elif (len(exons) > 1) and (sum(exons) > 200):
            multiexon.append(entry)

print("Removed single exon transcripts < 2000, and multi exon transcripts < 200")
